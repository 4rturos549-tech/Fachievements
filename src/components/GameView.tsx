...
const TROPHY_COLOR: Record<TrophyType, string> = { platinum: '#e8d5ff', gold: '#f5a623', silver: '#b0bec5', bronze: '#cd7f32' };
const TROPHY_BG: Record<TrophyType, string> = { platinum: 'rgba(232,213,255,0.08)', gold: 'rgba(245,166,35,0.1)', silver: 'rgba(176,190,197,0.08)', bronze: 'rgba(205,127,50,0.08)' };
const TROPHY_BORDER: Record<TrophyType, string> = { platinum: 'rgba(232,213,255,0.2)', gold: 'rgba(245,166,35,0.2)', silver: 'rgba(176,190,197,0.15)', bronze: 'rgba(205,127,50,0.15)' };
const TROPHY_LABEL: Record<TrophyType, string> = { platinum: 'Platino', gold: 'Oro', silver: 'Plata', bronze: 'Bronce' };

function TrophyIcon({ type, size = 16 }: { type: string; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={TROPHY_COLOR[type as TrophyType] || '#888'} strokeWidth="1.5">
...
type TrophyFilter = TrophyType | 'all';

export default function GameView({ session, manifest, meta, igdbId }: GameViewProps) {
  ...
  const missablesInPlaythrough = manifest?.playthroughs?.[activePlaythrough]?.zones
    ?.flatMap(z => z.steps)
    ?.filter(s => s.type === 'missable').length ?? 0;

  const totalMissables = manifest?.playthroughs
    ?.flatMap(p => p.zones.flatMap(z => z.steps))
    ?.filter(s => s.type === 'missable').length ?? 0;

  const achievements: Achievement[] = manifest?.achievements ?? [];
  const filtered = trophyFilter === 'all' ? achievements : achievements.filter(a => a.type === trophyFilter);
  const counts = {
  ...
  const tabs = [
    { key: 'info' as const, label: 'Info', icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>, badge: null as number | null },
    { key: 'logros' as const, label: 'Logros', icon: <TrophyIcon type="gold" size={13} />, badge: null as number | null },
    {
      key: 'guia' as const,
      label: 'Guía Platino',
  ...
  return (
    <div>
      ...
      {activeTab === 'guia' && (
        <div>
          {manifest && manifest.playthroughs.length > 0 ? (
              ...
              {guideMode === 'partidas' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    {manifest.playthroughs.map((p, i) => {
                      const pMissables = p.zones.flatMap(z => z.steps).filter(s => s.type === 'missable').length;
                      return (
