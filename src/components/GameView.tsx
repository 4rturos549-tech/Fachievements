...
const TROPHY_BG: Record<string, string> = {
  platinum: 'rgba(232,213,255,0.08)',
  gold:     'rgba(245,166,35,0.1)',
  silver:   'rgba(176,190,197,0.08)',
  bronze:   'rgba(205,127,50,0.08)',
};
const TROPHY_BORDER: Record<string, string> = {
  platinum: 'rgba(232,213,255,0.2)',
  gold:     'rgba(245,166,35,0.2)',
  silver:   'rgba(176,190,197,0.15)',
...
function TrophyIcon({ type, size = 16 }: { type: string; size?: number }) {
  const color = TROPHY_COLOR[type] || '#888';
  return (
...
export default function GameView({ session, manifest, meta }: { session: Session | null; manifest: any; meta: any }) {
  ...
  const tabs = [
    ...
    { key: 'guia'   as const, label: 'Guía Platino', icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg> },
  ];
  ...
  return (
    <div>
      {/* Session bar */}
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '1.5rem', gap: '0.75rem', alignItems: 'center' }}>
        {session ? (
          <>
            <span style={{ fontSize: '0.7rem', color: '#333', fontFamily: "'DM Mono', monospace" }}>{session.user.email}</span>
            <a href="/perfil" style={{ background: 'transparent', border: '1px solid #2a2a2a', color: '#555', fontSize: '0.7rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '0.4rem 0.8rem', borderRadius: '5px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", textDecoration: 'none' }}>Mi perfil</a>
          </>
        ) : (
      ...
      {/* Info del juego si hay manifest */}
      {manifest?.info && (
        ...
        <div>
          {manifest?.playthroughs?.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              {/* Toggle modo */}
              <div style={{ display: 'flex', background: '#0d0d0d', border: '1px solid #1e1e1e', borderRadius: '8px', padding: '3px', gap: '3px', alignSelf: 'flex-start' }}>
                {([['partidas', 'Por partida'], ['pasos', 'Paso a paso']] as const).map(([key, label]) => (
                  <button key={key} onClick={() => setGuideMode(key)} style={{ padding: '0.45rem 1rem', borderRadius: '6px', fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', border: 'none', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", transition: 'all 0.2s', background: guideMode === key ? '#f5a623' : 'transparent', color: guideMode === key ? '#000' : '#444' }}>
                    {label}
                  </button>
                ))}
              </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    {manifest.playthroughs.map((p: any, i: number) => (
                      <button key={i} onClick={() => setActivePlaythrough(i)} style={{ padding: '0.45rem 1rem', borderRadius: '6px', fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.05em', border: `1px solid ${activePlaythrough === i ? 'rgba(245,166,35,0.4)' : '#222'}`, background: activePlaythrough === i ? 'rgba(245,166,35,0.1)' : 'transparent', color: activePlaythrough === i ? '#f5a623' : '#444', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", transition: 'all 0.2s' }}>
                        {p.title}
                      </button>
                    ))}
                  </div>
                  {/* Summary de la partida */}
                  ...
                  <Tracker session={session} manifest={manifest} playthroughIndex={activePlaythrough} mode="partida" />
                </div>
              )}

              {guideMode === 'pasos' && (
                <Tracker session={session} manifest={manifest} playthroughIndex={0} mode="all" />
              )}
            </div>
          ) : (
