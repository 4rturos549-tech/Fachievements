...
import type { Manifest, Step, Zone, StepType } from '../lib/types';
...
export default function Tracker({ session, manifest, playthroughIndex, mode, spoilerMode, searchQuery, igdbId }: TrackerProps) {
  ...
  useEffect(() => {
    ...
    supabase.from('completed_steps').select('step_id').eq('user_id', session.user.id)
      .then(({ data, error }) => {
        if (!error && data) setCompletedSteps(data.map((r: { step_id: string }) => r.step_id));
        setIsLoading(false);
      });
  }, [session]);

  const toggleStep = async (stepId: string, type: StepType) => {
  ...
  const allCheckable = manifest.playthroughs.flatMap(p =>
    p.zones.flatMap(z => z.steps.filter(s => s.type !== 'tip'))
  );
  const totalSteps = allCheckable.length;
  const doneCount = completedSteps.filter(id => allCheckable.some(s => s.id === id)).length;
  const progress = totalSteps > 0 ? Math.round((doneCount / totalSteps) * 100) : 0;

  const matchesSearch = (step: Step) => {
  ...
  const renderZones = (zones: Zone[]) => {
    ...
    return zonesWithResults.map(zone => (
...
interface StepItemProps {
  ...
  disabled: boolean;
  onToggle: (id: string, type: StepType) => void;
  spoilerMode: boolean;
  searchQuery: string;
}
